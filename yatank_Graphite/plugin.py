...
class GraphiteUploaderPlugin(AbstractPlugin, AggregateResultListener):
    ...
    def configure(self):
        ...
        if self.address == "":
            ...
            self.log.info("testing3154: Graphite: configure: prefix: %s" % self.prefix)
    ...
    def aggregate_second(self, data):
        ...
        self.log.info("testing3154: GraphiteUploader: aggregate_second: data: %s" % str(data))
        if self.graphite_client:
            ...
            self.graphite_client.submit(results, data.time)
...
class GraphiteClient(object):
    ...
    def submit(self, results, tstmp):
        ...
        try:
            ...
            self.log.debug("testing3154: GraphiteClient: prefix: %s", self.prefix)
            for metric in results.keys():
                self.log.debug("testing3154: GraphiteClient.submit: "
                               "tstmp: %s, metric: '%s',"
                               "value: '%s'"
                               % (tstmp, metric,
                                  str(results[metric])))
                sock.sendall(
                    "%s.%s\t%s\t%d\n" %
                    (self.prefix, metric, results[metric],
                     int(time.mktime(tstmp.timetuple())))
                )
            sock.close()
